...
//$_POST['path_new'] = 'http://yourlogo.online';
$_POST['url_old'] = 'http://yourlogo.site';
//$_POST['url_new'] = 'http://yourlogo.online/' . $_POST['wp_user'];
//$_POST['siteurl'] = 'http://yourlogo.online/shop/' . $_POST['wp_user'];
$_POST['fullsearch'] = 1;
...
//UnZip the Archive file(PreProcess)
...
if (@is_dir($GLOBALS['CURRENT_ROOT_PATH'])) {
    @rmdir($GLOBALS['CURRENT_ROOT_PATH']);
}

$prev_log = "";
if (class_exists('ZipArchive')) {
    $target	 = $GLOBALS['CURRENT_ROOT_PATH'];
    $zip	 = new ZipArchive();
...
$db_file_size = filesize($root_path . '/database.sql');
...
$sql_file_copy_status   = file_put_contents($root_path . '/' . $GLOBALS['SQL_FILE_NAME'], $sql_file);
...
{
	$sql_file_size = DUPX_U::readableByteSize(filesize($root_path . '/database.sql'));
