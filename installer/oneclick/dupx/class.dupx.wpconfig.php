...
class DUPX_WPConfig
{
	...
	public static function updateStandard()
	{
		...
		if (!file_exists($wpconfig_path)) return;
		...
		if (!is_writable($wpconfig_path)) {
			if (file_exists($wpconfig_path)) {
				chmod($wpconfig_path, 0644) ? DUPX_Log::info('File Permission Update: wp-config.php set to 0644') : DUPX_Log::info('WARNING: Unable to update file permissions and write to wp-config.php.  Please visit the online FAQ for setting file permissions and work with your hosting provider or server administrator to enable this installer.php script to write to the wp-config.php file.');
			} else {
